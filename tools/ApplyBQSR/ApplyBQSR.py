...
def run_md(lines):
    ...
    plasmaid = lines[1][:20]
    flag = lines[2]
    p = subprocess.Popen(["java", "-Djava.library.path=/usr/local/lib/", "-jar", "/home/tahmad/bulk/apps/gatk/gatk.jar", "ApplyBQSR", "-R", "/dev/shm/tahmad/hg19/hg19.fasta", "-I", "/home/tahmad/bulk/apps/picard/headers/header_"+''.join(Chr)+''.join(".bam"), "-L","/home/tahmad/bulk/apps/bsqr/beds/"+''.join(Chr)+''.join(".bed"), "-bqsr", "/home/tahmad/bulk/apps/bsqr/"+''.join(Chr)+''.join(".table"), "-O", "/home/tahmad/bulk/apps/bsqr/"+''.join(Chr)+''.join("_applyBSQR.bam"), "Chr="+''.join(Chr), "Plasma="+''.join(plasmaid), "Flag="+''.join(flag)], stdout=subprocess.PIPE)
...
if __name__ == '__main__':
    ...
    if os.path.exists("/home/tahmad/bulk/apps/objIDsJavaQual.txt"):
        os.remove("/home/tahmad/bulk/apps/objIDsJavaQual.txt")
